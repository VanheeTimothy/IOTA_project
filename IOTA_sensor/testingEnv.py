...
#
# trytes = b'GWHCDJLT9I9TEWOLQDYRRKVTOFYLOMNKFUAKHKYBKKATQYBJPWYJVAOCPLULWGUZFZNZZPYRLZUBEIWLY'
#
#
# text = iota.Transaction.from_tryte_string(trytes)
#
# print(vars(text))
...
class IotaMamExample(IotaCommandLineApp):
    """
    Shows how to integrate the ``mam.client.js`` Javascript library into
    a Python script, until MAM functionality is implemented in PyOTA.
    In order to execute this script, you must install Node and the
    ``mam.client.js`` library.
    See https://github.com/iotaledger/mam.client.js for more
    information.
    """
    ...
    def execute(self, api, **arguments):
        ...
        if message_file:
            with codecs.open(message_file, 'r', message_encoding) as f_:
                message = f_.read()
        ...
        # Generating the encrypted message may take a little while, so
        # we should provide some feedback to the user so that they know
        # that their input is being processed (this is especially
        # important if the user typed in their message, so that they
        # don't press ^D again, thinking that the program didn't
        ...
        filter_ = f.FilterRunner(
            f.Required | f.Unicode | f.JsonDecode | f.Array | f.FilterRepeater(
    ...
    def create_argument_parser(self):
        ...
        parser.add_argument(
            ...
            help=(
                ...
                'If not provided, you will be prompted for the message '
                'via stdin.'
            ),
        )
        ...
        parser.add_argument(
            ...
            help=(
                'If set, resulting transactions will be sent to stdout '
                'instead of broadcasting to the Tangle.'
            ),
        )
